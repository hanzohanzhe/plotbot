    ...
    for job_id, task_details in JOBS.items():
        if task_details["status"] == "PENDING":
            ...
            return {
                ...
                "chat_id": task_details["chat_id"] # <-- 新增：返回 chat_id
            }
    return {"job_id": None, "prompt": None, "chat_id": None} # <-- 新增：没有任务时也返回 chat_id
...
async def update_task(update: TaskUpdateRequest):
    ...
    # 如果任务完成，且 worker.py 直接发送了文件，这里就不需要再发链接了
    if update.status == "COMPLETED":
        # 假设 worker 成功发送文件后，这里不需要再向用户发送链接
        # 而是可以发送一个确认消息
        send_telegram_message(job["chat_id"], f"🎉 您的任务 `{update.job_id}` 已完成！文件已通过机器人直接发送给您。")
